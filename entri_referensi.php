...
<!--Header-part-->
<div id="header">
  <h1><a href="entri_referensi.php">Entri Class</a></h1>
</div>
<!--close-Header-part--> 
          ...
          <ul class="thumbnails">
            <div class="btn-icon-pg">
              <ul>
                <!--Looping-->
                <?php
                  $query_data_program = "select * from tb_program order by id_program desc";
                ...
                ?>
                    <li class="span2"> 
                      ...
                      <form action="" method="post">
                        <button type="submit" value="<?php echo $r_dt_program['id_program'];?>" name="edit_menu" class="btn btn-success btn-mini"><i class='icon-pencil'></i>&nbsp;&nbsp;Edit &nbsp;&nbsp;</button>
                        <button type="submit" value="<?php echo $r_dt_program['id_program'];?>" name="hapus_menu" class="btn btn-mini btn-danger"><i class='icon icon-trash'></i>&nbsp; Hapus</button>
                      </form>
                    </li>
                  <?php
                    }
                    if(isset($_REQUEST['hapus_menu'])){
                      ...
                      $id_program = $_REQUEST['hapus_menu'];

                      $query_lihat = "select * from tb_program where id_program = $id_program";
                      $sql_lihat = mysqli_query($conn, $query_lihat);
                      $result_lihat = mysqli_fetch_array($sql_lihat);
                      if(file_exists('gambar/'.$result_lihat['gambar_program'])){
                        //echo $result_lihat['gambar_program'];
                        unlink('gambar/'.$result_lihat['gambar_program']);
                      }
                      $query_hapus_program = "delete from tb_program where id_program = $id_program";
                      $sql_hapus_program= mysqli_query($conn, $query_hapus_program);
                      if($sql_hapus_program){
                    ...
                    if(isset($_REQUEST['edit_menu'])){
                      //echo $_REQUEST['hapus_menu'];
                      $id_program = $_REQUEST['edit_menu'];
                      $_SESSION['edit_menu'] = $id_program;
...
<div class="row-fluid">
  <div id="footer" class="span12"> <?php echo date('Y'); ?> &copy; People Power Academy <a href="#">by IpatNyy_</a> </div>
</div>
