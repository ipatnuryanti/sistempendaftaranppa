        ...
        <div class="widget-box span12">
          ...
          <div class="widget-content">
            <div class="alert alert-info alert-block">
              ...
              Silahkan cek email anda untuk melakukan transaksi pembayaran !
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-invoice-full">
              ...
              <tbody>
                <?php
                  $no_order_fiks = 1;
                  $query_order_fiks = "select * from tb_pesan natural join tb_program where id_user = $id and status_pesan != 'sudah'";
                ...
                <tr>
                  ...
                  <td><?php echo $r_order_fiks['nama_program'];?></td>
    ...
    <div class="row-fluid">
                ...
                <ul>
                  ...
                  <?php
                    ...
                    while($r_dt_pesan = mysqli_fetch_array($sql_lihat_pesan)){
                      array_push($pesan, $r_dt_pesan['id_program']);
                    }

                    $query_data_program = "select * from tb_program where stok > 0 order by id_program desc";
                    $sql_data_program = mysqli_query($conn, $query_data_program);
                    $no_program = 1;

                    while($r_dt_program = mysqli_fetch_array($sql_data_program)){
                  ?>
                      <li class="span2"> 
                        <a> <img src="gambar/<?php echo $r_dt_program['gambar_program']?>" alt="" > </a>
                        <div class="actions">
                          <a class="lightbox_trigger" href="gambar/<?php echo $r_dt_program['gambar_program'];?>"><i class="icon-search"></i>&nbsp;Lihat</a> 
                        </div>
                        <table class="table table-bordered">
                          <tbody>
                            <tr>
                              <td><?php echo $r_dt_program['nama_program'];?></td>
                            </tr>
                            <tr>
                              <td>Harga</td>
                              <td>Rp. <?php echo $r_dt_program['harga'];?>,-</td>
                            </tr>
                            <tr>
                              <td>Stok</td>
                              <td><?php echo $r_dt_program['stok'];?> Orang</td>
                            </tr>
                          </tbody>
                        </table>
                        <form action="" method="post">
                          <?php
                            if(in_array($r_dt_program['id_program'], $pesan)){
                          ?>
                              <button type="submit" value="<?php echo $r_dt_program['id_program'];?>" name="tambah_pesan" class="btn btn-danger btn-mini" disabled>
                                <i class='icon-shopping-cart'></i>&nbsp;&nbsp;Telah dipesan &nbsp;&nbsp;
                              </button>
                          <?php
                            } else {
                          ?>
                              <button type="submit" value="<?php echo $r_dt_program['id_program'];?>" name="tambah_pesan" class="btn btn-success btn-mini">
                    ...
                    <?php
                      ...
                      if(isset($_REQUEST['tambah_pesan'])){
                        ...
                        $id_program = $_REQUEST['tambah_pesan'];

                        $query_tambah_pesan = "insert into tb_pesan values('', '$id', '', '$id_program', '', '')";
        ...
        <div class="widget-box">
          ...
          <div class="widget-content nopadding">
            <table class="table table-bordered table-striped">
              ...
              <tbody>
              <?php
                $query_draft_pesan = "select * from tb_pesan natural join tb_program where id_user = $id and status_pesan != 'sudah'";
                $sql_draft_pesan = mysqli_query($conn, $query_draft_pesan);

                while($r_draft_pesan = mysqli_fetch_array($sql_draft_pesan)){
              ?>
                <tr class="odd gradeX">
                  <td><span id="<?php echo "nama".$r_draft_pesan['id_pesan']; ?>"><?php echo $r_draft_pesan['nama_program'];?></span></td>
                  <input id="<?php echo "harga".$r_draft_pesan['id_pesan']; ?>" class="span8" type="hidden" value="<?php echo $r_draft_pesan['harga'];?>"/>
                  <td>
                    <center>
                      <input id="<?php echo "jumlah".$r_draft_pesan['id_pesan']; ?>" class="span8" name="jumlah<?php echo $r_draft_pesan['id_program']; ?>" type="number" value="" placeholder="" onchange="return operasi()"/>
                    </center>
                  </td>
                  <td>
                    <form action="" method="post">
      ...
      <?php
          ...
          if(isset($_POST['proses_pesan'])){
            ...
            $query_ubah_jumlah = "select * from tb_pesan left join tb_program on tb_pesan.id_program = tb_program.id_program where id_user = $id and status_pesan != 'sudah'";
            $sql_ubah_jumlah = mysqli_query($conn, $query_ubah_jumlah);
            while($r_ubah_jumlah = mysqli_fetch_array($sql_ubah_jumlah)){
              $tahu = $r_ubah_jumlah['id_program'];
              $tempe = $_POST['jumlah'.$tahu];
              $id_pesan = $r_ubah_jumlah['id_pesan'];
              $query_stok = "select * from tb_program where id_program = $tahu";
              $sql_stok = mysqli_query($conn, $query_stok);
              $result_stok = mysqli_fetch_array($sql_stok);
              $sisa_stok = $result_stok['stok'] - $tempe;
              //echo $tempe;
              $query_proses_ubah = "update tb_pesan set jumlah = $tempe, id_order = $id_ordernya where id_program = $tahu and id_user = $id and status_pesan != 'sudah'";
              $query_kurangi_stok = "update tb_program set stok = $sisa_stok where id_program = $tahu";
