          ...
          <div class="widget-content nopadding" >
            <table class="table table-bordered table-invoice-full">
              ...
              <?php
                ...
                $query_lihat_kelas = "select * from tb_program";
                $sql_lihat_kelas = mysqli_query($conn, $query_lihat_kelas);

              ?>
              <tbody>
              <?php
                while($r_lihat_kelas = mysqli_fetch_array($sql_lihat_kelas)){
              ?>
                <tr>
                  <td><center><?php echo $no++;?>.</center></td>
                  <td><?php echo $r_lihat_kelas['nama_program'];?></td>
                  <td><center><?php echo $r_lihat_kelas['stok'];?></center></td>
                  <td>
                    <center>
                      <?php
                        $id_program = $r_lihat_kelas['id_program'];
                        $query_lihat_stok = "select * from tb_stok left join tb_pesan on tb_stok.id_pesan = tb_pesan.id_pesan left join tb_program on tb_pesan.id_program = tb_program.id_program where status_cetak = 'belum cetak'";
                        $query_jumlah = "select sum(jumlah_terjual) as jumlah_terjual from tb_stok left join tb_pesan on tb_stok.id_pesan = tb_pesan.id_pesan where id_program = $id_program and status_cetak = 'belum cetak'";
                  ...
                  <td style="text-align: right">Rp. <?php echo $r_lihat_kelas['harga'];?> ,-</td>
                  <td style="text-align: right">Rp. 
                      ...
                      <?php
                        ...
                        if($result_jumlah['jumlah_terjual'] != 0 || $result_jumlah['jumlah_terjual'] != null || $result_jumlah['jumlah_terjual'] != ""){
                          //echo $result_jumlah['jumlah_terjual'];
                          $jml = $result_jumlah['jumlah_terjual'] * $r_lihat_kelas['harga'];
                          echo $jml;
                        } else {
                          $jml = $result_jumlah['jumlah_terjual'] * $r_lihat_kelas['harga'];
                          echo $jml;
...
<script type="text/javascript">
  // This function is called from the pop-up kelass to transfer to
  // a different page. Ignore if the value returned is a null string:
  function goPage (newURL) {

      // if url is empty, skip the kelas dividers and reset the kelas selection to default
      if (newURL != "") {
      
          // if url is "-", it is this page -- reset the kelas:
          if (newURL == "-" ) {
              resetkelas();            
          } 
...
// resets the kelas selection upon entry to this page:
function resetkelas() {
   document.gokelas.selector.selectedIndex = 2;
}
</script>
</body>
</html>
