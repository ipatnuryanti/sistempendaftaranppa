...
if(isset ($_SESSION['username'])){
  ...
  $harga = "";
  $stok = "";
  $gambar_program = "no_image.png";

  if(isset($_SESSION['edit_menu'])){
    $id = $_SESSION['edit_menu'];
    $query_data_edit = "select * from tb_program where id_program = $id";
    $sql_data_edit = mysqli_query($conn, $query_data_edit);
    $result_data_edit = mysqli_fetch_array($sql_data_edit);

    $id_program = $result_data_edit['id_program'];
    $nama_program = $result_data_edit['nama_program'];
    $harga = $result_data_edit['harga'];
    $stok = $result_data_edit['stok'];
    $gambar_program = $result_data_edit['gambar_program'];
  }
      ...
      <div class="widget-box span6">
        ...
        <div class="widget-content" >
          <form action="" method="post" class="form-horizontal" accept-charset="UTF-8" enctype="multipart/form-data">
            <div class="control-group">
              ...
              <div class="controls">
                <?php 
                  if(isset($_SESSION['edit_menu'])){
                ?>
                  <input name="" type="text" value="<?php echo $nama_program; ?>" class="span11" placeholder="Nama program" disabled=""/>
                  <input name="nama_program" type="hidden" value="<?php echo $nama_program; ?>" class="span11" placeholder="Nama program"/>
                <?php
                  } else {
                ?>
                  <input name="nama_program" type="text" value="" class="span11" placeholder="Nama program"/>
                <?php
                  }
                ?>
              </div>
            ...
            <div class="control-group">
              <label class="control-label">Gambar program :</label>
              <div class="control-group">
                <div class="controls">
            ...
            <div class="control-group">
              ...
              <div class="control-group">
                <div class="controls">
                  <img src="gambar/<?php echo $gambar_program;?>" id="previewne" class="rounded border p-1" style="width:110px; height:70px;">
                </div>
              </div>
            </div>
          ...
          <?php
            if(isset($_POST['tambah_menu'])){
              ...
              $nama_baru = $_POST['nama_program'].".".$name;
              ...
              if($stok > 0){
                $status_program = 'tersedia';
              } else {
                $status_program = 'habis';
              }
              //echo "<br>";
              //echo $nama_user . " || " . $username . " || " . $password . " || " . $id_level . " || " . $status;
              //echo "<br></br>";
              $query_tambah_program = "insert into tb_program values ('','$nama_program','$harga','$stok','$status_program','$gambar')";
              $sql_tambah_program= mysqli_query($conn, $query_tambah_program);
              if($sql_tambah_program){
            ...
            if(isset($_POST['ubah_menu'])){
              ...
              $query_ubah_program = "update tb_program set nama_program = '$nama_program', harga = '$harga', stok = '$stok', status_program = '$status_program' where id_program = '$id_program'";;
              $sql_ubah_program = mysqli_query($conn, $query_ubah_program);
              ...
              if($gbr != "" || $gbr != null){
                ...
                $name = pathinfo($_FILES["gambar"]["name"], PATHINFO_EXTENSION);
                $nama_baru = $_POST['nama_program'].".".$name;
                unlink('gambar/'.$gambar_program);
                move_uploaded_file($tmp_name, $direktori."/".$nama_baru);
                $gambar = $nama_baru;

                $query_ubah_gambar = "update tb_program set gambar_program = '$gambar' where id_program = '$id_program'";;
